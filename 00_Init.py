...
def plugin_loaded():
    try:
        ...
        entropy_CA_CERTS_B = bytes(CA_CERTS, "UTF-8")

        if not os.path.isdir(entropy_cache_path):
            os.makedirs(entropy_cache_path)

        if not os.path.isfile(entropy_certs_path):
            with open(entropy_certs_path, "wb") as f:
                f.write(entropy_CA_CERTS_B)

        else:
            sha1_lhex = hashlib.sha1(entropy_CA_CERTS_B).hexdigest()
            sha1_rhex = hashlib.sha1(open(entropy_certs_path, "rb").read()).hexdigest()

            if sha1_lhex != sha1_rhex:
                with open(entropy_certs_path, "wb") as f:
                    f.write(entropy_CA_CERTS_B)
        ...
        sublime.error_message("".join([
                "ERROR!\n\n",
                "Error generating CA CERTS file!",
            ]))
